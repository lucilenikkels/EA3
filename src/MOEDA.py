...
class MOEDA:
    def __init__(self, populationSize, numberOfVariables, numberOfEvaluations, fitnessFunction, 
               ...
               randomSeed=42, elitistArchive = False):
        ...
        self.useArchive = elitistArchive
    ...
    def plotFronts(self, generation, title='fronts.png'):
        ...
        N_colors = len(self.fronts[generation-1])
        cmap = matplotlib.colors.LinearSegmentedColormap.from_list('Custom cmap', cmaplist, cmap.N)
        bounds = np.linspace(0, N_colors, N_colors+1)
        norm = matplotlib.colors.BoundaryNorm(bounds, cmap.N)
        
        x,y,tags = [], [], []
        for i, front in enumerate(self.fronts[generation-1]):
        ...
        #plt.savefig('fronts_gen%d.png' % generation)
        plt.show()

    def get_best_front(self, generation):
        x, y, tags = [], [], []
        for i, front in enumerate(self.fronts[len(self.fronts) - 1]):
            obj1 = [solution.fitness[0] for solution in front]
            obj2 = [solution.fitness[1] for solution in front]
            x += obj1
            y += obj2
        ...
        x, y, tags = np.array(x), np.array(y), np.array(tags)
        ma = np.ma.masked_where(tags == 0, tags)
        mask = ma.mask
        x_front = x[mask]
        y_front = y[mask]
